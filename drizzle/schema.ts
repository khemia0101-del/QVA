...
/**
 * Leads table for storing credit partnership application submissions
 */
...
/**
 * Mailing list table for email capture signups
 */
export const mailingList = mysqlTable("mailing_list", {
  id: int("id").autoincrement().primaryKey(),
  email: varchar("email", { length: 320 }).notNull().unique(),
  source: varchar("source", { length: 50 }), // 'blog' or 'podcast'
  createdAt: timestamp("createdAt").defaultNow().notNull(),
});
